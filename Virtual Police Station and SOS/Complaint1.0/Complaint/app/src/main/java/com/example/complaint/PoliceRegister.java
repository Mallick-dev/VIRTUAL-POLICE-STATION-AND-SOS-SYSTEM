...
public class PoliceRegister extends AppCompatActivity {
        ...
        mAuth.createUserWithEmailAndPassword(email, pass).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful()){
                    ...
                    databaseReference= FirebaseDatabase.getInstance().getReference().child("police").child(currentUserId);
                    ...
                    databaseReference.updateChildren(userInfo).addOnCompleteListener(new OnCompleteListener() {
                        ...
                        public void onComplete(@NonNull Task task) {
                            if(task.isSuccessful()){
                                Toast.makeText(PoliceRegister.this, "Data Inserted Successfully", Toast.LENGTH_SHORT).show();
                            }else{
                                Toast.makeText(PoliceRegister.this, task.getException().toString(), Toast.LENGTH_SHORT).show();
                            }
                            finish();
    ...
    public void onStart() {
        ...
        if(currentUser != null){
            Intent intent=new Intent(getApplicationContext(),PoliceMain.class);
